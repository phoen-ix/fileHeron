...
- **Enrolled TOTP is challenged after OIDC and after a passkey too** (2026-08-15). Both paths called `finalize_successful_login` directly, so switching 2FA on did nothing at all for anyone who signed in that way while the account page said it was on - the audit found the SSO half, the passkey half was the same defect one route over. A passkey is not automatically a second factor: the ceremony may be one possession factor. **Since 2026-09-06 a UV-verified assertion IS the second factor**: `/webauthn/begin` requests `UserVerificationRequirement.REQUIRED` when TOTP is enabled, `webauthn.authenticate_complete` returns `PasskeyAuthResult(user, user_verified)`, and only `user_verified=False` earns the pending token. Because a passkey can now stand in for TOTP, `register/begin` is step-up gated (`verify_password_or_403`) like `/2fa/disable`. Before this the SPA could not consume the pending token at all (`stores/auth.ts` fed `undefined` to `setAccessToken`), and the "Use passkey" button renders only after `TOTP_REQUIRED` - so it was dead for exactly the users it was shown to, with no test on either side.
...
- **The direct-upload ceiling is the SERVER's, read from `/api/config-public` at sign-in** (`upload_worker.set_direct_upload_limit`, 2026-09 audit). It was a build-time 100 MB, so an instance whose admin lowered `uploads.max_direct_bytes` refused every file between the two limits with 413 while the SPA, which reads the same field, streamed them - the SPA's own audit-#2 defect, one surface over. The public config is fetched by the sign-in WORKER (each `login_window._attempt` returns it) and handed to `AppController._on_signed_in`; the controller's inline fetch is a fallback only, because an HTTP call on the Tk thread is the class `test_no_other_blocking_api_call_remains_on_the_tk_thread` exists to stop (it knew two call names and missed `patch_locale` for a year; it scans every `api_pkg.*` call now).
- **`upload_direct` goes through `ApiClient.request()`, never `_http.post`.** `request()` is the only place a 401 becomes refresh-and-replay, and uploads are queued now (`MAX_PARALLEL_UPLOADS`), so one that starts after the access token expired is an ordinary case, not an edge. The replay re-reads the file: httpx seeks it to 0 and `_ProgressReader.seek` rewinds the counter with it.
