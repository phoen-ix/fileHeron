...
toggle on /api/admin/crons (email-on-server-error feature)."""
...
async def test_cron_alert_on_failure_round_trip(make_user, client, login_as):
    ...
    await client.put(
        "/api/admin/settings/error-alerts",
        json={
            "enabled": True, "source_http_5xx": True, "recipients_mode": "admins",
            "custom_recipients": [], "cooldown_minutes": 15, "max_per_hour": 20,
        },
        headers={"Authorization": f"Bearer {token}"},
    )
