<template>
  <section class="webauthn-panel">
    ...
    <div v-if="!supported" class="fh-notice" role="alert" data-tone="error">
    ...
    <div v-else>
      <ul v-if="credentials.length > 0" class="cred-list">
        <li v-for="c in credentials" :key="c.id" class="cred-row">
          ...
          <button type="button" class="fh-btn-text danger" :disabled="busy" @click="onDelete(c.id)">
            {{ t('webauthn.remove') }}
          </button>
        </li>
      </ul>
  ...
  async function onRegister() {
    ...
    try {
      ...
      const cred = await performRegistration({
        ...
        excludeCredentials: (opts.excludeCredentials || opts.exclude_credentials) as
          { id: string; type: 'public-key'; transports?: string[] }[] | undefined,
        authenticatorSelection: (opts.authenticatorSelection || opts.authenticator_selection) as
          | {
