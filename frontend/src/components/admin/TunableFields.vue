<!-- Renders the registry tunables that belong on ONE admin page, with the
     same row layout, reset control and all-or-nothing save the Advanced page
     always had - because it IS the Advanced page's renderer, parameterised by
     `config/adminTunablePlacement.ts`. The data comes from and goes back to
     `/api/admin/settings/advanced`, the single writer for every registry knob;
     this component only decides which of its items this page shows.

     `route` is the page's route name; only tunables placed there render.
     `groups` narrows further, for a page that spreads its tunables over
     several sections (Files & transfers: Uploads vs Downloads). -->
<template>
  <div class="tunable-fields">
    <div v-if="loading" class="loading">{{ t('common.loading') }}</div>

    <form v-else @submit.prevent="onSave">
      <p v-if="!visible.length" class="fh-field-help">{{ t('admin_advanced.none_here') }}</p>

      <section v-for="g in grouped" :key="g.group" class="group">
        <h2 v-if="showHeadings" class="group-h2">{{ groupLabel(g.group) }}</h2>
        <!-- The visible name is a sibling <span>, so it has to be associated
             ...
             security- or retention-relevant (audit #2). The id doubles as
             the anchor the Overview's setting search scrolls to. -->
      ...
      <div v-if="visible.length" class="actions">
...
import { placementFor } from '@/config/adminTunablePlacement'
import { useUiStore } from '@/stores/ui'

const props = defineProps<{
  /** Route name of the page this instance sits on. */
  route: string
  /** Optional narrowing to these registry groups. */
  groups?: string[]
  /** Render a heading per group. Defaults to "when more than one group". */
  headings?: boolean
}>()
...
const visible = computed(() =>
  items.value.filter(
    (it) =>
      placementFor(it.key, it.group) === props.route &&
      (!props.groups || props.groups.includes(it.group)),
  ),
)

// Stable group order (the registry's own); anything unknown falls to the end.
const GROUP_ORDER = [
  'sessions', 'rate_limits', 'security', 'retention', 'uploads', 'downloads',
  'updates', 'branding', 'storage', 'anomaly', 'error_alert',
]

const grouped = computed(() => {
  const by: Record<string, AdvancedSettingItem[]> = {}
  for (const it of visible.value) (by[it.group] ??= []).push(it)
...
const showHeadings = computed(() => props.headings ?? grouped.value.length > 1)

// True if any visible draft differs from the loaded effective value.
const dirty = computed(() => visible.value.some((it) => draft.value[it.key] !== it.value))
...
async function onSave() {
  saving.value = true
  errorMsg.value = null
  // Only send changed VISIBLE keys. A value equal to the env default is sent
  // as null (reset → delete the override).
...
.group { margin-bottom: var(--fh-space-4); }
.group-h2 {
...
.actions { margin-top: var(--fh-space-3); }
</style>
