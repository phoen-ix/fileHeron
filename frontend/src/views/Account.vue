<script setup lang="ts">
  ...
  import SectionQuickNav, { type QuickNavSection } from '@/components/SectionQuickNav.vue'
  ...
  import type { AdminNavCollapseMode, Locale, SessionRecord, TotpStatusResponse } from '@/types/api'
  ...
  async function saveDisplayName() {
    ...
    try {
      ...
      ui.pushToast(t('account.display_name_save_failed') + ' ' + describe(e), 'error')
    } finally {
  ...
  async function changeLandingPage(value: string | null) {
    ...
    try {
      ...
      ui.pushToast(t('account.landing.save_failed') + ' ' + describe(e), 'error')
    }
  }
  ...
  async function onRevokeOthers() {
    if (!(await ui.confirm({ message: t('account.session_revoke_others_confirm'), danger: true })))
      return
    try {
  ...
  <div class="account-layout">
    <div class="account-prose fh-prose">
      ...
      <section id="profile" class="account-section fh-rise" data-stagger="3">
        ...
        <div class="fh-field">
          <label class="fh-field-label" for="acc-display-name">{{
          ...
          <div v-if="nameDirty" class="display-name-actions">
            <button type="button" class="fh-btn" :disabled="nameSaving" @click="saveDisplayName">
        ...
        <div class="fh-field">
          ...
          >
            ...
            <option v-if="auth.user?.home_page_enabled" value="home">
      ...
      <!-- Admin sidebar collapse mode (admins only) -->
      <section v-if="auth.user?.role === 'admin'" id="admin-nav" class="account-section">
        <h2 class="account-h2">{{ $t('account.admin_nav.title') }}</h2>
        <div class="fh-field">
          <span id="acct-admin-nav-label" class="fh-field-label">{{
            $t('account.admin_nav.label')
          }}</span>
      ...
      <section v-if="auth.user?.can_change_own_email" id="email" class="account-section">
        ...
        <form @submit.prevent="changeEmail">
          ...
          <div class="fh-field">
            <label class="fh-field-label" for="acc-email-pw">{{
          ...
          <div v-if="emailError" class="fh-notice" role="alert" data-tone="error">
            {{ emailError }}
          </div>
      ...
      <section id="password" class="account-section">
        ...
        <form @submit.prevent="changePassword">
          ...
          <div v-if="pwError" class="fh-notice" role="alert" data-tone="error">{{ pwError }}</div>
      ...
      <section id="2fa" class="account-section">
        ...
        <div class="twofa-state">
          ...
          <div v-else-if="totpLoadFailed" class="fh-notice" role="alert" data-tone="error">
            ...
            <button type="button" class="fh-btn-text" @click="loadTotp">
              {{ $t('common.retry') }}
            </button>
          </div>
      ...
      <section id="sessions" class="account-section">
        ...
        <ul class="sessions">
          <SessionRow v-for="s in sessions" :key="s.id" :session="s" @revoke="revokeSession" />
        </ul>
        <button
