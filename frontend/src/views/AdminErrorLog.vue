<script setup lang="ts">
  ...
  import { ADMIN_LOG_MAX_PAGE, exportErrorCsv, listErrorLog } from '@/api/admin'
  ...
  <div class="fh-page" data-density="operator">
    ...
    <div class="filters">
      <input
        ...
        :placeholder="t('admin_error_log.filter.code')"
        :aria-label="t('admin_error_log.filter.code')"
      />
      <input
        v-model.trim="ip"
        class="fh-field-input"
        :placeholder="t('admin_error_log.filter.ip')"
      ...
      />
      <select v-model="source" class="fh-field-input" :aria-label="t('common.filter')">
      ...
      <input
        ...
        class="fh-field-input"
        type="datetime-local"
        :title="t('admin_error_log.filter.from')"
      ...
      <input
        ...
        class="fh-field-input"
        type="datetime-local"
        :title="t('admin_error_log.filter.to')"
        ...
        <tbody>
          <template v-for="r in items" :key="r.id">
            ...
            >
              ...
              <td>
                <span class="fh-pill" :data-state="statusTone(r.status_code)">{{
                  r.status_code
                }}</span>
                <span v-if="r.alerted" class="fh-pill mini" data-state="active">{{
                  t('admin_error_log.emailed')
                }}</span>
              </td>
              <td class="fh-mono">{{ r.code }}</td>
              <td class="fh-mono where">
                {{
                  r.source === 'worker' ? r.job_name : `${r.method ?? ''} ${r.path ?? ''}`.trim()
                }}
              </td>
            ...
            <tr v-if="expanded === r.id" class="detail-row">
              <td colspan="6">
                <dl class="detail">
                  <dt>{{ t('admin_error_log.detail.exception') }}</dt>
                  <dd class="fh-mono">{{ r.exception_type ?? '-' }}</dd>
                  <dt>{{ t('admin_error_log.detail.source') }}</dt>
                  <dd class="fh-mono">{{ r.source }}</dd>
                  <dt>{{ t('admin_error_log.detail.ip') }}</dt>
                  <dd class="fh-mono">{{ r.ip ?? '-' }}</dd>
                  <dt>{{ t('admin_error_log.detail.request_id') }}</dt>
                  <dd class="fh-mono">{{ r.request_id ?? '-' }}</dd>
                  <dt>{{ t('admin_error_log.detail.user') }}</dt>
                  <dd class="fh-mono">
                    {{
                      r.user_id !== null
                        ? `#${r.user_id}${r.auth_via ? ` (${r.auth_via})` : ''}`
                        : '-'
                    }}
                  </dd>
                  <dt>{{ t('admin_error_log.detail.signature') }}</dt>
                  <dd class="fh-mono">{{ r.signature }}</dd>
                  <dt>{{ t('admin_error_log.detail.message') }}</dt>
                  <dd class="msg-full">{{ r.message ?? '-' }}</dd>
                </dl>
              </td>
            </tr>
          </template>
