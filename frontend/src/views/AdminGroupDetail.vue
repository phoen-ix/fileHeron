<script setup lang="ts">
  ...
  import { addMembers, deleteGroup, getGroup, removeMember, updateGroup } from '@/api/groups'
  ...
  async function onRemove(userId: number) {
    if (!group.value) return
    if (!(await ui.confirm({ message: t('admin_group_detail.remove_confirm'), danger: true })))
      return
    try {
  ...
  async function onDeleteGroup() {
    if (!group.value) return
    if (!(await ui.confirm({ message: t('admin_group_detail.delete_confirm'), danger: true })))
      return
    try {
...
<template>
  <div class="fh-page" data-density="operator">
    ...
    <template v-else-if="group">
      ...
      <button v-if="!editingName" type="button" class="fh-btn-text" @click="editingName = true">
      ...
      <div class="add-member">
        ...
        <ul v-if="userResults.length > 0 || userSearching" class="add-results">
          ...
          <li
            v-for="u in userResults.filter(
              (r) => !(group?.members ?? []).some((m) => m.user_id === r.user_id),
            )"
            :key="u.user_id"
            class="add-result"
          >
            ...
            <button type="button" class="fh-btn-text" :disabled="adding" @click="onAdd(u)">
      ...
      <ul class="member-list">
        <li v-for="m in group.members" :key="m.user_id" class="member-row">
          ...
          <button type="button" class="fh-btn-text danger" @click="onRemove(m.user_id)">
