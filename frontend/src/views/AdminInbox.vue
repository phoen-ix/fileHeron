  ...
  async function onFetchNow() {
    ...
    try {
      ...
      if (data.ok && data.skipped) {
        ...
        ui.pushToast(
          t('admin_inbox.fetch_done', { n: data.ingested ?? 0 }, data.ingested ?? 0),
          'success',
        )
      } else if (data.ok) {
        ui.pushToast(
    ...
    <div v-else class="fh-table-scroll">
      <table class="fh-table">
        ...
        <tbody>
          ...
          >
            <td>
              <span class="badge" :data-tone="classTone[m.classification]">{{
                t(`admin_inbox.tag_${m.classification}`)
              }}</span>
            </td>
            <td>
            ...
            <td>
              {{ m.subject }}
              <span v-if="m.has_attachments" class="clip" :title="t('admin_inbox.has_attachments')"
                >📎</span
              >
            </td>
