...
<template>
  <div class="msg-page" data-density="operator">
    ...
    <article v-else-if="msg" class="msg">
      <header class="head">
        <span class="badge" :data-tone="msg.classification">{{
          t(`admin_inbox.tag_${msg.classification}`)
        }}</span>
        <h1 class="subject">{{ msg.subject }}</h1>
        <dl class="meta">
          <div>
            <dt>{{ t('admin_inbox.from') }}</dt>
            <dd>
              {{ msg.sender_name ? `${msg.sender_name} <${msg.sender_email}>` : msg.sender_email }}
            </dd>
          </div>
          <div v-if="msg.to_addr">
            <dt>{{ t('admin_inbox.to') }}</dt>
            <dd>{{ msg.to_addr }}</dd>
          </div>
          <div>
            <dt>{{ t('admin_inbox.received') }}</dt>
            <dd>{{ formatDate(msg.received_at || msg.created_at) }}</dd>
          </div>
        </dl>
      </header>
      ...
      <div class="toolbar">
        ...
        <button
          ...
          @click="view = 'text'"
        >
          {{ t('admin_inbox.view_text') }}
        </button>
        <span class="spacer" />
        <button type="button" class="fh-btn-text" @click="setStatus('archived')">
          {{ t('admin_inbox.archive') }}
        </button>
        <button type="button" class="fh-btn-text danger" @click="onDelete">
          {{ t('common.delete') }}
        </button>
      </div>
      ...
      <section v-if="msg.attachments.length" class="attachments">
        ...
        <ul>
          <li v-for="a in msg.attachments" :key="a.id" class="att">
            ...
            <span class="fh-mono att-meta"
              >{{ formatBytes(a.size_bytes) }} · {{ a.content_type || '-' }}</span
            >
            <button
            ...
            <span v-else class="att-state" :data-tone="a.av_state">{{
              t(`admin_inbox.av_${a.av_state}`)
            }}</span>
          </li>
        </ul>
      </section>
