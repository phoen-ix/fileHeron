<script setup lang="ts">
  ...
  const { items, total, page, pageSize, loading, errorMsg, load } = usePaginatedList<IpBlockRow>(
  ...
  function isLive(row: IpBlockRow): boolean {
    ...
    // does nothing.
    return row.released_at === null && parseServerDate(row.expires_at).getTime() > Date.now()
  }
  ...
  <div class="fh-page" data-density="operator">
    ...
    <section class="card">
      <div class="sec-head">
        ...
        <button v-if="hasLiveBlocks" type="button" class="fh-btn-text danger" @click="onReleaseAll">
      ...
      <div v-else-if="!items.length" class="empty">
        ...
        <button v-if="historyTotal > 0" type="button" class="fh-btn-text" @click="status = 'all'">
