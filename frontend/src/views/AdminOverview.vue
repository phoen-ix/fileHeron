    ...
    <section class="ov-section" aria-labelledby="ov-attention-h">
      ...
      <div class="ov-tiles">
        <RouterLink
          ...
          class="ov-tile"
          :data-state="
            tile.warn && typeof tile.count === 'number' && tile.count > 0 ? 'warn' : undefined
          "
        >
          <b
            class="ov-tile-n"
            :title="tile.count === null ? t('admin_overview.unavailable') : undefined"
            >{{ tile.count === undefined ? '…' : tile.count === null ? '–' : tile.count }}</b
          >
    ...
    <section class="ov-section" aria-labelledby="ov-status-h">
      ...
      <div class="ov-health">
        ...
        <span v-for="c in checks" :key="c.key" class="ov-health-item">
          <span class="dot" :class="c.ok === null ? 'unknown' : c.ok ? 'ok' : 'warn'"></span
          >{{ t(c.labelKey) }}
        </span>
      </div>
    </section>
...
<script setup lang="ts">
  ...
  const tiles = ref<Tile[]>([
    {
      key: 'inbox',
      labelKey: 'admin_overview.tile_unread_inbox',
      to: { name: 'admin-inbox' },
      count: undefined,
      warn: true,
    },
    {
      key: 'quarantine',
      labelKey: 'admin_overview.tile_quarantined',
    ...
    {
      key: 'blocks',
      labelKey: 'admin_overview.tile_live_blocks',
      to: { name: 'admin-ip-blocks' },
      count: undefined,
      warn: false,
    },
    {
      key: 'failed',
      labelKey: 'admin_overview.tile_failed_tasks',
      to: { name: 'admin-scheduled-tasks' },
      count: undefined,
      warn: true,
    },
    {
      key: 'approvals',
      labelKey: 'admin_overview.tile_pending_approvals',
      to: { name: 'approvals' },
      count: undefined,
      warn: false,
  ...
  onMounted(() => {
    ...
    void loadTile(
      'quarantine',
      async () => (await adminListFiles({ state: 'infected', page_size: 1 })).data.total,
    )
    void loadTile(
      'blocks',
      async () => (await listIpBlocks({ status: 'active', page_size: 1 })).data.total,
    )
    if (auth.user?.can_approve_shares) {
      void loadTile(
        'approvals',
        async () => (await listPendingApprovals({ page_size: 1 })).data.total,
      )
    }
    void (async () => {
      try {
        ...
        setCount(
          'failed',
          data.crons.reduce((n, c) => n + c.last_24h.failure, 0),
        )
      } catch {
