              ...
              <tr v-for="it in grp.items" :key="it.name">
                ...
                <td class="sched-cell">
                  <div class="cell-row">
                    <select
                      v-model="it.kind"
                      class="fh-field-input kind"
                      :aria-label="t('common.filter')"
                    >
                      <option value="interval">
                        {{ t('admin_scheduled_tasks.kind_interval') }}
                      </option>
                    ...
                    <span v-else class="sched-input">
                      <input
                        v-model="it.daily_time"
                        type="time"
                        class="fh-field-input time"
                        :aria-label="t('common.filter')"
                      />
                    </span>
                  </div>
                ...
                <td class="status-cell">
                  <span v-if="it.last_status" class="fh-pill" :data-tone="statusTone(it)">{{
                    it.last_status
                  }}</span>
                  <span v-else class="muted">-</span>
                  <span class="counts fh-mono">
                    <span class="ok">{{ it.last_24h.success }}</span
                    >/<span class="bad">{{ it.last_24h.failure }}</span>
                  </span>
                  <span v-if="it.last_run_at" class="last fh-mono">{{
                    formatDate(it.last_run_at)
                  }}</span>
                </td>
                <td class="fh-mono next">
                  {{
                    it.enabled ? formatDate(it.next_run_at) : t('admin_scheduled_tasks.disabled')
                  }}
                </td>
                <td class="actions">
