<script setup lang="ts">
  ...
  async function onRevoke(s: AdminSessionRow) {
    ...
    if (!(await ui.confirm({ message: t('admin_sessions.revoke_confirm', { who }), danger: true })))
      return
  ...
  async function onRevokeAll(s: AdminSessionRow) {
    ...
    if (
      !(await ui.confirm({
        message: t('admin_sessions.revoke_all_confirm', { who }),
        danger: true,
      }))
    )
      return
          ...
          <tr v-for="s in items" :key="s.id" :class="{ inactive: !s.is_active }">
            <td>
              ...
              <div class="fh-mono row-hint">
                {{ s.user_email || t('admin_sessions.deleted_user') }}
              </div>
            </td>
            <td>
              ...
              <span v-if="!s.is_active" class="fh-pill" data-state="danger">
                {{
                  s.revoked_at
                    ? t('admin_sessions.state_revoked')
                    : t('admin_sessions.state_expired')
                }}
              </span>
            </td>
