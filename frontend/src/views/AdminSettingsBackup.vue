<script setup lang="ts">
  ...
  async function onPreview() {
    ...
    preview.value = null
    try {
      const { data } = await previewBackupImport(
        importFile.value,
  ...
  <div class="policy-page" data-density="operator">
    ...
    <section class="card">
      ...
      <fieldset class="group">
        ...
        <label
          v-for="mode in ['passphrase', 'ciphertext', 'exclude'] as const"
          :key="mode"
          class="toggle"
        >
      ...
      <fieldset v-if="secretMode === 'passphrase'" class="group">
        ...
        />
        <p v-if="passphrase && !passphraseOk" class="fh-notice" role="alert" data-tone="error">
    ...
    <section class="card">
      ...
      <div v-if="importError" class="fh-notice" role="alert" data-tone="error">
        {{ importError }}
      </div>
          ...
          <li v-if="preview.admins_installed?.length" class="danger">
            {{
              t(
                ...
                { n: preview.admins_installed.length },
                preview.admins_installed.length,
              )
            }}:
