<script setup lang="ts">
  ...
  async function onReset() {
    if (!activeSlug.value || !activeLocale.value) return
    if (!(await ui.confirm({ message: t('admin_email_templates.reset_confirm'), danger: true })))
      return
    saving.value = true
    errorMsg.value = null
    try {
  ...
  async function onTestSend() {
    ...
    try {
      ...
      if (data.ok) {
        ui.pushToast(
          t('admin_email_templates.test_ok_toast', { to: data.sent_to ?? '' }),
          'success',
        )
      } else {
        ui.pushToast(
          data.hint || data.error_message || t('admin_email_templates.test_failed_toast'),
          'warn',
        )
      }
    } catch (err) {
      ...
      <section v-if="activeSlug && activeLocale" class="tpl-editor">
        <header class="editor-head">
          ...
          <span class="fh-pill" :data-state="customized ? 'active' : undefined">
            {{
              customized
                ? t('admin_email_templates.badge_customized')
                : t('admin_email_templates.badge_default')
            }}
          </span>
        </header>
        ...
        <template v-else>
          ...
          <details
            ...
            @toggle="placeholderOpen = ($event.target as HTMLDetailsElement).open"
          >
            ...
            <ul class="placeholder-list">
              <li v-for="p in activePlaceholders" :key="p.token">
                ...
                <span class="ph-desc">
                  {{ p.description }}
                  <span v-if="p.required" class="ph-required">{{
                    t('admin_email_templates.required_tag')
                  }}</span>
                </span>
              </li>
            </ul>
          ...
          <section v-if="preview" class="preview">
            ...
            <p class="preview-subject">
              <span class="fh-field-label">{{
                t('admin_email_templates.preview_subject_label')
              }}</span>
              {{ preview.subject }}
            </p>
            <iframe
