  ...
  <div class="policy-page" data-density="operator">
    ...
    <form v-else class="policy-form" @submit.prevent="onSave">
      ...
      <section v-if="showAllowlist" class="allowlist">
        <h2 class="form-h2">{{ t('admin_public_link_policy.allowlist_heading') }}</h2>
        <p class="fh-field-help">{{ t('admin_public_link_policy.allowlist_help') }}</p>

        <div class="fh-field">
          <span class="fh-field-label">{{ t('admin_public_link_policy.users_label') }}</span>
        ...
        <div v-if="availableGroups.length > 0" class="fh-field">
          <span class="fh-field-label">{{ t('admin_public_link_policy.groups_label') }}</span>
            ...
            <li v-for="g in availableGroups" :key="g.id">
              <label class="group-check">
                ...
                <span v-if="g.is_company_inbox" class="fh-pill">
                  {{ t('admin_public_link_policy.groups_inbox') }}
                </span>
              </label>
            </li>
          </ul>
