<script setup lang="ts">
  ...
  function applyPresetDefaults() {
    ...
    helperFields.value = Object.fromEntries(p.issuer_template_fields.map((f) => [f.key, '']))
    ...
    try {
      if (isEdit.value) {
        await updateProvider(providerId.value, {
          ...
          client_secret: form.value.client_secret === '' ? null : form.value.client_secret,
...
<template>
  <div class="sso-edit" data-density="operator">
    ...
    <form v-else class="sso-form" @submit.prevent="onSave">
      ...
      <template v-if="activePreset && activePreset.issuer_template_fields.length > 0">
        <label v-for="hf in activePreset.issuer_template_fields" :key="hf.key" class="fh-field">
      ...
      <div class="actions">
        ...
        <button type="button" class="fh-btn-ghost fh-btn" :disabled="testing" @click="onTest">
