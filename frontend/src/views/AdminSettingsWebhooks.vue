<script setup lang="ts">
  ...
  async function onDelete(w: WebhookItem) {
    if (
      !(await ui.confirm({
        message: t('admin_webhooks.delete_confirm', { name: w.name }),
        danger: true,
      }))
    )
      return
...
<template>
  <div class="fh-page" data-density="operator">
    ...
    <div v-if="justCreatedSecret" class="created-box fh-rise">
      ...
      <div class="actions">
        <button type="button" class="fh-btn-text" @click="copySecret">
          {{ t('admin_webhooks.copy') }}
        </button>
        <button type="button" class="fh-btn-text" @click="justCreatedSecret = null">
          {{ t('admin_webhooks.acknowledged') }}
        </button>
      </div>
    </div>
    ...
    <section class="card create-form">
      ...
      <label class="fh-field">
        ...
        <input
          ...
          :placeholder="t('admin_webhooks.name_ph')"
        />
      </label>
      <label class="fh-field">
      ...
      <li v-for="w in items" :key="w.id" class="hook card">
        <div class="hook-head">
          ...
          <div class="hook-actions">
            <button type="button" class="fh-btn-text" @click="onTest(w)">
              {{ t('admin_webhooks.test') }}
            </button>
            <button type="button" class="fh-btn-text" @click="toggleDeliveries(w)">
              {{ t('admin_webhooks.deliveries') }}
            </button>
            <button type="button" class="fh-btn-text" @click="onToggleActive(w)">
              {{ w.active ? t('admin_webhooks.disable') : t('admin_webhooks.enable') }}
            </button>
            <button type="button" class="fh-btn-text danger" @click="onDelete(w)">
              {{ t('admin_webhooks.delete') }}
            </button>
          </div>
        </div>
        <pre class="hook-url fh-mono">{{ w.url }}</pre>
        <div class="hook-events">
          <span v-for="e in w.event_types" :key="e" class="event-tag" :title="e">{{
            eventLabel(e)
          }}</span>
        </div>
        ...
        <div v-if="openDeliveries === w.id" class="deliveries">
          ...
          <p v-else-if="deliveries.length === 0" class="fh-field-help">
            {{ t('admin_webhooks.no_deliveries') }}
          </p>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="d in deliveries" :key="d.id">
                  ...
                  <td>
                    <span class="fh-pill" :data-state="pillTone[d.status]">{{ d.status }}</span>
                  </td>
                  <td class="num fh-mono">{{ d.response_code ?? '-' }}</td>
                  <td class="num fh-mono">{{ d.attempts }}</td>
                  <td class="num">
                    <button type="button" class="fh-btn-text" @click="onRetry(d)">
                      {{ t('admin_webhooks.retry') }}
                    </button>
                  </td>
                </tr>
              </tbody>
