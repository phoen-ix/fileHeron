  ...
  async function onCheckNow() {
    ...
    try {
      ...
      if (data.latest_version && data.latest_version !== status.value?.version.running) {
        ...
        ui.pushToast(t('admin_system.update.check_now_toast.none'), 'success')
      }
    } catch (err) {
      ui.pushToast(describe(err), 'error')
    } finally {
      ...
      try {
        ...
        if (data.state === 'healthy' || data.state === 'failed' || data.state === 'rolled_back') {
          ...
          if (data.state === 'healthy') {
            ui.pushToast(t('admin_system.update.toast.done', { tag: data.target_tag }), 'success')
    ...
    <template v-if="status">
      ...
      <section v-if="status.version" class="card">
        <div class="card-header">
          ...
          <button type="button" class="btn-secondary" :disabled="checking" @click="onCheckNow">
        ...
        <dl class="kv-grid">
          ...
          <dd>
            ...
            <span
              v-if="
                status.version.last_success_at &&
                status.version.last_success_at !== status.version.last_check_at
              "
              class="sha"
            >
              ·
              {{
                t('admin_system.version.last_success', {
                  when: fmtTime(status.version.last_success_at),
                })
              }}
            </span>
            <a
          ...
          <template v-if="status.version.last_check_error">
            ...
            <dd>
              <span class="error-line">{{ status.version.last_check_error }}</span>
            </dd>
          </template>
        </dl>
        ...
        <div v-if="status.version.update_available" class="update-banner">
          <div class="banner-text">
            <strong>{{
              t('admin_system.version.update_available_title', { v: status.version.latest })
            }}</strong>
            <p v-if="status.version.release_published_at" class="banner-sub">
              {{
                t('admin_system.version.published', {
                  when: fmtTime(status.version.release_published_at),
                })
              }}
            </p>
          </div>
        ...
        <div v-if="pendingUpdate && !jobInFlight" class="pending-banner">
          <div class="banner-text">
            ...
            <p class="banner-sub">
              {{
                t('admin_system.update.postpone.deadline', {
                  when: fmtTime(pendingUpdate.deadline_iso),
                })
              }}
            </p>
          </div>
        ...
        <div v-if="updaterStatus?.rollback_target && !jobInFlight" class="rollback-row">
      ...
      <section class="card">
        ...
        <dl class="kv-grid">
          ...
          <dd>
            <span :class="statusClass(status.live.redis.status)">{{
              status.live.redis.status
            }}</span>
            <span v-if="status.live.redis.error" class="error-line">{{
              status.live.redis.error
            }}</span>
          </dd>
      ...
      <section class="counters">
        ...
        <div class="counter">
          ...
          <span class="counter-label">{{
            t('admin_system.counters.email_undeliverable_24h')
          }}</span>
        </div>
      </section>
      ...
      >
        <div class="fh-modal fh-modal--small">
          ...
          <form @submit.prevent="submitConfirm(false)">
            ...
            <div v-if="confirmError" class="fh-notice" data-tone="error" role="alert">
              {{ confirmError }}
            </div>
            <div class="form-actions">
              <button type="submit" class="btn-primary" :disabled="submitting || !passwordInput">
                {{
                  submitting
                    ? t('common.loading')
