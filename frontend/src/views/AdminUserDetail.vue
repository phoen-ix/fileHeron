<script setup lang="ts">
  ...
  async function onDeleteFile(f: AdminFileItem) {
    ...
    if (
      !(await ui.confirm({
        message: t('admin_user_detail.files_delete_confirm', { name: f.filename }),
        danger: true,
      }))
    )
      return
  ...
  async function onRevokeAllSessions() {
    ...
    if (
      !(await ui.confirm({
        message: t('admin_user_detail.sessions_revoke_all_confirm'),
        danger: true,
      }))
    )
      return
    ...
    try {
      ...
      ui.pushToast(
        t('admin_sessions.revoked_all_toast', { n: data.revoked }, data.revoked),
        'success',
      )
      await loadSessions()
    } catch (err) {
...
<template>
  <div class="fh-page" data-density="operator">
    ...
    <div v-else-if="loadError" class="fh-notice" role="alert" data-tone="error">
      {{ loadError }}
    </div>
    ...
    <template v-else-if="user">
      ...
      <form class="edit-form" @submit.prevent="onSave">
        ...
        <div v-if="savingError" class="fh-notice" role="alert" data-tone="error">
          {{ savingError }}
        </div>
        <div class="actions">
      ...
      <div class="security-row">
        <div>
          <p class="fh-field-help">
            {{ t('admin_user_detail.created') }}: {{ formatDate(user.created_at) }}
          </p>
          <p class="fh-field-help">
            {{ t('admin_user_detail.last_login') }}: {{ formatDate(user.last_login_at) }}
          </p>
        </div>
        <div class="actions">
          <button
      ...
      <form class="change-email-form" @submit.prevent="onChangeEmail">
        ...
        <div v-if="changeEmailError" class="fh-notice" role="alert" data-tone="error">
          {{ changeEmailError }}
        </div>
        <div class="actions">
          <button
        ...
        <li v-for="s in sessions" :key="s.id" class="session-item">
          <div class="session-info">
            ...
            <span class="fh-mono session-meta">
              ...
              <span
                >{{ t('admin_sessions.col.last_active') }}: {{ formatDate(s.last_used_at) }}</span
              >
            </span>
          </div>
          <button
        ...
        <li v-for="f in files" :key="f.file_id" class="file-item">
          <div class="file-info">
            ...
            <span class="fh-mono file-meta">
              ...
              <span class="fh-pill" :data-state="f.state === 'clean' ? 'active' : 'warn'">{{
                f.state
              }}</span>
          ...
          >
            ...
            <span class="fh-mono file-meta">
              ...
              <span
                class="fh-pill"
                :data-state="
                  m.status === 'sent' ? 'active' : m.status === 'queued' ? 'warn' : 'danger'
                "
                >{{ m.status }}</span
              >
