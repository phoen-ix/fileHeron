  ...
  <div class="fh-page approvals" data-density="operator">
    ...
    <ul v-else class="queue">
      <li v-for="s in items" :key="s.id" class="queue-row">
        <div class="meta">
          <RouterLink :to="{ name: 'share-detail', params: { id: s.id } }" class="subject">
            {{ s.effective_subject || t('approvals.no_subject') }}
          </RouterLink>
          <div class="sub fh-mono">
            <span v-if="s.sender"
              >{{ t('approvals.from', { name: s.sender.display_name }) }} ·
            </span>
            {{ t('approvals.file_count', { n: s.file_count }, s.file_count) }}
            · {{ formatBytes(s.total_size_bytes) }} · {{ formatDate(s.created_at) }}
          </div>
        </div>
        <RouterLink :to="{ name: 'share-detail', params: { id: s.id } }" class="fh-btn review-cta">
