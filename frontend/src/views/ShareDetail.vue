<script setup lang="ts">
  ...
  const isOwner = computed(() => share.value?.created_by_id === auth.user?.id)
  const canManage = computed(() => isOwner.value || auth.user?.role === 'admin')
  ...
  const addedAwaitingReview = computed(() => (share.value?.files_awaiting_review ?? []).length)
  ...
  async function onApprove() {
    ...
    try {
      const { data } = await approveShare(share.value.id, share.value.content_fingerprint)
      share.value = data
  ...
  async function onReject() {
    ...
    try {
      const { data } = await rejectShare(share.value.id, rejectReason.value.trim() || null)
      share.value = data
  ...
  const addAllDone = computed(
    () =>
      ...
      addUpload.items.value.every((i) => i.state === 'done' || i.state === 'finalizing'),
  )
  ...
  const addRegistrar = createSettledRegistrar({
    ...
    register: async (id, fileIds) => {
      const { data } = await registerFilesAdded(id, {
  ...
  const totalSize = computed(() =>
    share.value ? share.value.files.reduce((acc, f) => acc + f.size_bytes, 0) : 0,
  )
    ...
    <template v-else-if="share">
      ...
      <div v-if="editingLimit" class="edit-expiry-panel">
        <label class="fh-field">
          <input v-model="newLimitClear" type="checkbox" />
          <span class="toggle-name" style="margin-left: 8px">{{
            t('share_detail.download_limit_clear')
          }}</span>
        </label>
      ...
      <div v-if="isOwner && share.state === 'pending_approval'" class="approval-box">
      ...
      <div v-if="isOwner && share.state === 'rejected'" class="approval-box">
        <p class="fh-notice" role="alert" data-tone="error">
        ...
        <div class="approver-buttons">
          <button type="button" class="fh-btn" :disabled="resubmitting" @click="onResubmit">
      ...
      >
        ...
        <p v-if="share.public_link_summary" class="fh-notice" data-tone="warn">
          {{
        ...
        <div v-if="!showRejectForm" class="approver-buttons">
          <button type="button" class="fh-btn" :disabled="approving" @click="onApprove">
            {{ approving ? t('common.loading') : t('approvals.approve_cta') }}
          </button>
          <button type="button" class="fh-btn-ghost fh-btn" @click="showRejectForm = true">
        ...
        <form v-else class="reject-form" @submit.prevent="onReject">
          ...
          <div class="approver-buttons">
            <button type="submit" class="fh-btn-danger fh-btn" :disabled="rejecting">
      ...
      <div v-if="showAddFiles" class="edit-expiry-panel add-files-panel">
        ...
        <div class="edit-expiry-actions">
          ...
          >
            {{
              addingBusy || addUpload.isActive.value
                ? t('common.loading')
                : t('share_detail.add_files_cta')
            }}
          </button>
          <button
            type="button"
      ...
      <PublicLinkPanel v-if="isOwner && share.state === 'active'" :share-id="share.id" />
