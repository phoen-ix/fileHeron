...
- `docker/backend/Dockerfile` — multi-stage Python 3.12-slim, install pyproject deps, run `uvicorn`. Match `REDACTED/docker/backend/Dockerfile` shape.
...
- `backend/app/main.py` — FastAPI app factory with `lifespan` (DB pool + Redis pool init), middleware mount, router registration. Port from `REDACTED/backend/app/main.py` minus the SPA-mount (we serve SPA from nginx).
- `backend/app/config.py` — `Settings(BaseSettings)`, fail-fast `sys.exit()` on missing prod secrets. Match `REDACTED/backend/app/config.py`.
- `backend/app/database.py` — engine `pool_pre_ping=True`, `pool_recycle=3600`; `Base(DeclarativeBase)`; `get_db()` generator. Port verbatim from `REDACTED/backend/app/database.py`.
...
3. **Cookie scoping** — refresh cookie scoped to `/api/auth` (not `/`) so future tusd/upload routes never carry the cookie. Match `REDACTED/backend/app/routers/auth.py` cookie config.
